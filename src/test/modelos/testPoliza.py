...
class TestPoliza:
    def __init__(self, usuario: TestUsuarioCliente, periodo_carencia: datetime, tipo: str, 
    			copagos: float, mensualidad: str, servicios_excluidos: List[str]):
    ...
    def __eq__(self, otra):
    	...
    	return (self.__titular == otra.get_titular() ) and (self.__periodo_carencia == otra.get_periodo_carencia()) and (self.__tipo == otra.get_tipo()) and (self.__copagos == otra.get_copagos()) and (self.__mensualidad == otra.get_mensualidad()) and (self.__servicios_excluidos == otra.get_servicios_excluidos())
...
def test_compare_poliza():
	...
	t1 = TestPoliza(u, fecha, "Total", 35.99, 103.0, ["TAC", "Apendicitis"])
	t2 = TestPoliza(u2, fecha, "Total", 35.99, 103.0, ["TAC", "Apendicitis"])
	t3 = TestPoliza(u2, fecha2, "Básico", 35.99, 103.0, ["TAC", "Apendicitis"])
